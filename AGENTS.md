...
- **Preise kommen NIE aus einem Sprachmodell** (ADR-002). Marktwert nur aus
  echten Belegen; sonst zeigt die App ehrlich „Wert unbekannt". Unter drei
  Vergleichsangeboten gibt es keinen Preisvorschlag. Wer eine Preisschätzung
  in einen Prompt einbaut, bricht `tests/test_pricing.py`.
...
- **Die beiden besten Preisquellen sind aus Lizenzgründen im Code AUS:**
  ...
  „gerade gedrosselt".
- **Nichts geht ohne Freigabe live** — aber Vorsicht: Der Testmodus ist
  derzeit AUS. Maßgeblich ist NICHT `DRY_RUN` in der `.env`, sondern der
  Datenbankwert `kv['dry_run']` (per Telegram-Befehl `/dryrun` gesetzt); der
  steht aktuell auf `false`. Jedes Veröffentlichen geht also wirklich zu eBay
  und kostet echte Gebühren. Stand prüfen mit:
  `sqlite3 -readonly data.db "SELECT value FROM kv WHERE key='dry_run'"`.
  Selbst im Dry-Run legt SERO echtes Inventar und ein echtes (unveröffent-
  lichtes) Offer bei eBay an — nur `publishOffer` entfällt.
- **Das eigene freigestellte Foto ist immer das Hauptbild**, nie ein
  Katalogbild.
- **Bild-Standard: Warp pur, keine Kosmetik.** Kein Weichzeichnen, kein
  Aufhellen einzelner Bereiche, kein künstlicher Hintergrund. Festgeschrieben
  im Docstring von `web/cardscan.py` und in `tests/test_render_standard.py`,
  die den Quelltext prüfen. Änderungen nur nach ausdrücklicher Zusage von
  Sven — drei Anläufe sind bereits gescheitert.
- **Entwürfe mit Status `published`, `ended` oder `dry_run_done` nie
  anfassen.** Daran hängt echtes Geld.
